// Boomerang Video Background — captures frames from the video, then ping-pongs them
...
function BoomerangVideoBg({ src, className }) {
  ...
  useEffect(() => {
    ...
    const vfcLoop = () => { captureFrame(); if (capturing && video.requestVideoFrameCallback) video.requestVideoFrameCallback(vfcLoop); };
    const onEnded = () => {
      capturing = false;
...
window.BoomerangVideoBg = BoomerangVideoBg;
